      ...
      <form className={styles.form}>
          <div className="loginForm">
              <div className={styles.inputWrapper}>
                  <input value={emailValue} placeholder={`${t('mail')}`} type="text" onChange={(e) => setEmail(e.target.value)}/>
                  <input value={passwordValue} placeholder={`${t('password')}`} type="text" onChange={(e) => setPassword(e.target.value)}/>
              </div>
              <div className={styles.footerForm}>
                  <div className={styles.checkbox}>
                      <input checked={isChecked} type="checkbox" onChange={() => setChecked(true)}/>
                      <label onClick={() => setChecked(!isChecked)}>{t('savePassText')}
                      </label>
                  </div>
                  <button onClick={() => navigate('/recover-pass')}>
                      {t('recoverPass')}
                  </button>
              </div>
