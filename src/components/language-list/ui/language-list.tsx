...
export enum LangView {
    Simple = 'Simple', // just horizontal list
    Popup ='Popup', // the first two elements + remaining in "+count" form
    Detail = 'Detail' // flagCountry + nameCountry
}
...
const LanguageList = (props: LanguageListProps) => {
  ...
  return (
      ...
      <div className={styles.main}>
            {languages.slice(0, 2).map((item, i) => (
                <LanguageItem key={`${item}-${i}`} flagImg={item} />
                )).concat(
                    <div ref={popupRef}>
