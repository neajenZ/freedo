...
(function (PaymentType) {
    ...
    PaymentType["MASTER_CARD"] = "MASTER_CARD";
    ...
    return (<div className={styles.container}>
        <h4 className={styles.container__title}>Платёжные системы</h4>
          <div className={styles.payments}>
              {preparePayments.map((item, i) => (<div className={styles.payments__pay} key={`${item}-${i}`}>
                      {paymentConfig[item]}
                  </div>))}
              {payments.length > 6 &&
            <Button onClick={() => setShowAllPayments(!showAllPayments)} className={styles.payments__showBtn}>{showAllPayments ? `+${payments.length - preparePayments.length}` : '^^^'}</Button>}
          </div>
      </div>);
};
